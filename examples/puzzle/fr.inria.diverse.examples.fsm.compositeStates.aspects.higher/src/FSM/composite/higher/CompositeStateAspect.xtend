...
import static extension FSM.composite.higher.CompositeStateAspect.*
...
class StateMachineAspect{
	...
	def public Transition highestConflictingTransition(ArrayList<Transition> newActiveTransitions, 
		...
		if(selectedTransition.source instanceof State){
			...
			var ArrayList<Vertex> parents = new ArrayList<Vertex>()
...
class CompositeStateAspect { 
	...
	def public void exitState(Hashtable<String, Object> context){
		_self.states.forEach[ _vertex |
			if(_vertex instanceof State){
				val State _state = _vertex as State
