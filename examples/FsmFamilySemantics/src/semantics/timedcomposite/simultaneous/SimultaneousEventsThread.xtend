...
import static extension semantics.timedcomposite.simultaneous.CompositeStateAspect.*
import static extension semantics.timedcomposite.simultaneous.StateAspect.*
import static extension semantics.timedcomposite.simultaneous.StateMachineAspect.*
import static extension semantics.timedcomposite.simultaneous.TransitionAspect.*
...
class SimultaneousEventsThread extends Thread {
	...
	Context context
	ArrayList<timedcompositefsm.fsm.State> currentState
	...
	new (String _event, StateMachine _stateMachine, Context _context){
		...
		context = _context
		currentState = new ArrayList<timedcompositefsm.fsm.State>()
	 ...
	 */
	override run(){
		var EList<timedcompositefsm.fsm.State> attendedStates = new BasicEList<timedcompositefsm.fsm.State>()
		
		for(timedcompositefsm.fsm.State _state : stateMachine.currentState){
		...
		for(timedcompositefsm.fsm.State _state : currentState){
			for(Transition transition : _state.outgoing){
	...
	def private void processTransitions(Context context){
		...
		var EList<timedcompositefsm.fsm.State> toAdd = new BasicEList<timedcompositefsm.fsm.State>()
		var EList<timedcompositefsm.fsm.State> toRemove = new BasicEList<timedcompositefsm.fsm.State>()
