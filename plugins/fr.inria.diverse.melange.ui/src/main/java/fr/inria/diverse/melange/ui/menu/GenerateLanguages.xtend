...
import static fr.inria.diverse.melange.ui.menu.GenerateLanguages.*

class GenerateLanguages extends AbstractHandler {
		...
		try {
			new ProgressMonitorDialog(shell).run(true, true, new IRunnableWithProgress() {
				override run(IProgressMonitor monitor) throws InvocationTargetException, InterruptedException {
					...
					builder.generateLanguages(res, project, monitor)
				}
			})
